...
**중복은 계약이다 — at-least-once.** 같은 메시지가 두 번 이상 들어갈 수 있는 경로는 셋이다:
fsync와 삭제 사이에 프로세스가 죽는 경우, 그 사이 답장이 참조해 삭제에서 빠진 행이 다음 패스에
다시 기록되는 경우, 그리고 수동 `pager prune`이 훅의 자동 prune과 겹치는 경우.
...
**닫지 않은 경합이 하나 있다.** 수동 `pager prune`은 24시간 게이트를 우회하므로 훅의 자동 prune과
동시에 파일을 쓸 수 있고, 그 둘은 서로 직렬화되지 않는다. 겹치는 순간에 부분 쓰기까지 겹쳐야
드러나며, 결과는 손실이 아니라 깨진 줄 하나다.
